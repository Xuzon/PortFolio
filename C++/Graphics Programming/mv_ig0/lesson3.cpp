...
float Cz = 250.0;			//Distancia de la cámara
...
void TeclaModo()
{
	if (keys[VK_UP])
	{
		z = z - 10 * cos(Ry * 3.14/180.0);
		x = x - 10 * sin(Ry * 3.14/180.0);
	}
	if (keys[VK_DOWN])
	{
		z = z + 10 * cos(Ry * 3.14 / 180.0);
...
GLuint LoadBMPTexture(const char* imagepath)	// Load Bitmaps And Convert To Textures
{
	...
	// Lectura de informaci�n del encabezado del archivo
	...
	// Algunos archivos BMP tienen un mal formato, as� que adivinamos la informaci�n que falta
...
int InitGL(GLvoid)										// All Setup For OpenGL Goes Here
{
	...
	glOrtho(-1000.0, 1000.0, -1000.0, 1000.0, 2000.0, -2000.0);
	...
	if (texture[0] = LoadBMPTexture("Data/NeHe.bmp"))
	{
...
void DrawLightTeapot(float scale, bool solid)
{
...
int DrawGLScene(GLvoid)									// Here's Where We Do All The Drawing
{
	...
	glMatrixMode(GL_PROJECTION);						// Select The Projection Matrix
	glLoadIdentity();									// Reset The Projection Matrix
	gluPerspective(80, ancho / (float)alto, 10, 1000);
	//Make sure we are changing the modelview matrix
	glMatrixMode(GL_MODELVIEW);							// Select The Modelview Matrix
	glLoadIdentity();
	glTranslatef(0, -200, -Cz);
	glRotatef(-Ry, 0, 1, 0);
	glTranslatef(-x, -y, -z);
	//Luz después de rotar la cámara
	glLightfv(GL_LIGHT0, GL_POSITION, pos0);
	DibMalla(100.0);
	...
	//local object translate and rotate
	glTranslatef(0, 100, 0);
	glRotatef(90, 0, 1, 0);
	DrawLightTeapot(100, false);
	glPopMatrix();
