...
public class ClientAPI {
	...
	public static void main(String[] args) throws IOException{
		...
		String url;
		while(!salir){
			System.out.print("1) Crear noticia, 2) Conseguir noticia, 3) Salir");
	...
	static void setNoticia(ServiceClient sc,String titulo,String descripcion,String url,Options opts){
		try{
			...
			//sc.fireAndForget(setTitular(titulo)); //opción que nos da igual si llega o no
	...
	static String getNoticia(ServiceClient sc,Options opts){
		try{
			...
			sc.setOptions(opts);//set options
			OMElement ot = sc.sendReceive(getTitular());
			opts.setAction("urn:GetDescripcion");
			sc.setOptions(opts);//set options
			OMElement od = sc.sendReceive(getDescripcion());
			opts.setAction("urn:GetUrl");
			sc.setOptions(opts);//set options
			OMElement ou = sc.sendReceive(getUrl());
			String titular = "";
			String descripcion = "";
			String url = "";
			try{
				titular = ot.getFirstElement().getText();
				descripcion = od.getFirstElement().getText();
				url = ou.getFirstElement().getText();
